                        ...
                        <div class="card-body" style="width: 100%">
                            <div class="table-responsive">
                                <table class="table table-bordered" id="domaintablosu" width="100%" cellspacing="0">
                                    ...
                                    <tbody>
                                        <?php
                                        $domainsql = $db->prepare("SELECT * FROM users WHERE is_delete=0");
                                        $domainsql->execute();
                                        while ($userlist = $domainsql->fetch(PDO::FETCH_ASSOC)) {
                                        ?>
                                        <tr>
                                            <td><?php echo htmlspecialchars($userlist['id'] ?? ''); ?></td>
                                            <td><?php echo htmlspecialchars($userlist['user_name'] ?? ''); ?></td>
                                            <td><?php echo htmlspecialchars($userlist['user_phone'] ?? ''); ?></td>
                                            <td><?php echo htmlspecialchars($userlist['user_mail'] ?? ''); ?></td>
                                            <td>
                                                <form action="edit_user.php" method="get" style="display:inline;">
                                                    <input type="hidden" name="user_id"
                                                        value="<?php echo htmlspecialchars($userlist['id']); ?>">
                                                    <button type="submit"
